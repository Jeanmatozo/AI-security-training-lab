...
Run the full payload set:

```bash
python tools/fuzzer.py --payloads attacks/prompt-injection/payloads.json --output artifacts/results/fuzz_results_prompt_injection.json
```

Promote confirmed findings to evidence transcripts:

```bash
python tools/evidence_promoter.py --input artifacts/results/fuzz_results_prompt_injection.json --output evidence/transcripts/
```

Evidence files will be named `PI-YYYYMMDD-NNN.md` matching the payload category.

---

## RAG payload notes

For RAG-specific attacks see `attacks/rag-attacks/payloads.json` and run against the `rag-pipeline` environment on port `8001`:

```bash
python tools/fuzzer.py --payloads attacks/rag-attacks/payloads.json --output artifacts/results/fuzz_results_rag.json
```

Evidence files will be named `RAG-YYYYMMDD-NNN.md`.
...
|---|---|---|
| PI-018, PI-019 VULNERABLE | `LLM01-prompt-injection.md` | `chains/chain-01-rag-exfiltration.md` |
| PI-020 VULNERABLE | `LLM08-excessive-agency.md` | `chains/chain-02-agent-escalation.md` |
| PI-003 + PI-001/002 VULNERABLE | `LLM01-prompt-injection.md` | Multi-turn role confusion chain |